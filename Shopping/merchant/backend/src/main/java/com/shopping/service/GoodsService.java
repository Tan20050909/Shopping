...
public class GoodsService extends ServiceImpl<GoodsMapper, Goods> {
    ...
    public boolean updateGoods(Goods input) {
        if (input.getId() == null) return false;
        Goods existing = getById(input.getId());
        if (existing == null) return false;
        // 只允许更新业务字段，禁止透传审核/状态/商家字段
        ...
        if (input.getCategoryId() != null) existing.setCategoryId(input.getCategoryId());
        if (input.getShipFrom() != null) existing.setShipFrom(input.getShipFrom());
        if (input.getGoodsPic() != null) existing.setGoodsPic(input.getGoodsPic());
        if (input.getGoodsVideo() != null) existing.setGoodsVideo(input.getGoodsVideo());
        existing.setUpdateTime(LocalDateTime.now());
        // 已拒绝商品编辑后重新进入待审核
        if (existing.getAuditStatus() != null && existing.getAuditStatus() == 2) {
            existing.setAuditStatus(0);
            existing.setStatus(0);
            existing.setAuditRemark(null);
        }
        // 已通过/待审核商品保持原审核状态和上下架状态不变
    ...
    public boolean updateStatus(Long id, Integer status) {
        if (status == null || id == null) return false;
        Goods existing = getById(id);
        if (existing == null) return false;

        // 平台下架(status=3)商品：只允许提交复审(status=1→改为status=0/auditStatus=0)
        if (existing.getStatus() != null && existing.getStatus() == 3) {
            ...
            // 其他操作（包括 status=0）一律拒绝
            return false;
        }

        // 上架时必须审核通过
        if (status == 1 && (existing.getAuditStatus() == null || existing.getAuditStatus() != 1)) {
            return false;
        }
