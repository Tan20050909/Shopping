<template>
  <div class="login-page">
    <div class="login-top-tabs">
      ...
      <button class="top-tab" :class="{ active: activeRole === 'admin' }" type="button" @click="activeRole = 'admin'">平台管理员登录</button>
    </div>
    ...
    <div class="login-card">
      ...
      <section class="card-right">
        ...
        <div v-else-if="activeRole === 'merchant' && merchantTab === 'register'" class="form-wrap">
        ...
        <div v-else-if="activeRole === 'admin'" class="form-wrap">
          <el-form :model="adminLoginForm" label-position="top">
            <el-form-item label="管理员账号">
              <el-input v-model="adminLoginForm.username" placeholder="请输入管理员账号" />
            </el-form-item>
            <el-form-item label="密码">
              <el-input v-model="adminLoginForm.password" type="password" placeholder="请输入密码" show-password @keyup.enter="handleAdminLogin" />
            </el-form-item>
            <el-form-item class="btn-row">
              <el-button type="primary" class="primary-btn" @click="handleAdminLogin">登录</el-button>
            </el-form-item>
            <div class="link-row">
              <span class="muted">平台管理员专用登录</span>
            </div>
          </el-form>
        </div>
      </section>
...
const handleAdminLogin = async () => {
  ...
  let payload
  try {
    payload = await adminUserApi.login(adminLoginForm.value)
  } catch (error) {
    const msg = error?.response?.data?.message || error?.message || '登录失败'
    console.error('[admin-login] 请求异常:', error)
    ElMessage.error(msg)
    return
  }

  // adminUserApi 用的是商家端的 axios 实例，response 拦截器不会改数据
  // payload.data 就是后端返回的 { code, message, data }
  const res = payload?.data || {}
  console.log('[admin-login] 响应:', res)
  ...
  // 回跳地址：优先用 back 参数，没有则默认跳 admin 后台首页
  let back = backUrl.value
  if (!isValidAdminBackUrl(back)) {
    back = 'http://localhost:3002/dashboard'
  }

  const targetUrl = new URL(back)
  targetUrl.searchParams.set('adminToken', String(data.token))
  console.log('[admin-login] 跳转:', targetUrl.toString())
